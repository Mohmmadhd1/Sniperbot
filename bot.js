...
client.on('message', msg => {  //WESO#0001
    if (msg.content === 'كسمك') {  //WESO#0001
      msg.reply('**سيتم طردك من السيرفر بسبب السب من قبل @!♛ჯNἔἷᾗ • -🎇#9224**');  //WESO#0001
    }
  });


client.on('message', msg => {  //WESO#0001
    if (msg.content === 'كس امك') {  //WESO#0001
      msg.reply('**سيتم طردك من السيرفر بسبب السب من قبل @!♛ჯNἔἷᾗ • -🎇#9224**');  //WESO#0001
    }
  });
...
client.on('message',async message => {  //WESO#0001
  ...
  .setDescription(`s! \`${prefix}clan\`: نظام الكلانات هو نظام شبه مسلي ينمي التفاعل ويمكنك التحكم بالكلان تبعك بشكل كامل
  s! \`${prefix}clan info\`: لأظهار رسالة الأوامر ( هذه الرسالة ) ء
  s! \`${prefix}clan create\`: لأنشاء كلان بالأسم الذي تريده
  s! \`${prefix}clan invite\`: لدعوة شخص ما للكلان تبعك
  s! \`${prefix}clan join\`: للتقديم على دخول الكلان الذي تريده
  s! \`${prefix}clan promote\`: لأعطاء شخص بالكلان صلاحيات الادمن ( يتطلب صلاحية الادمن ) ء
  s! \`${prefix}clan demote\`: لأزالة صلاحية الادمن من عضو بالكلان ( صاحب الكلان فقط ) ء
  s! \`${prefix}clan ownership\`: لنقل ملكيةالكلان
  s! \`${prefix}clan leave\`: للخروج من الكلان الذي انت به
  s! \`${prefix}clan kick\`: لطرد عضو من الكلان ( يتطلب صلاحية الادمن ) ء
  s! \`${prefix}clan disband\`: لمسح الكلان من السستم ( صاحب الكلان فقط ) ء
  s! \`${prefix}clan stats\`: لعرض معلومات الكلان تبعك
  s! \`${prefix}clan list\`: يظهر لك اعضاء الكلان برسالة
  s! \`${prefix}clan accept\`: لقبول شخص وجعل الشخص يدخل الكلان ( يتطلب صلاحية الادمن ) ء
  s! \`${prefix}clan decline\`: لرفض شخص وعم جعل الشخص يدخل الكلان ( يطلب صلاحية الادمن ) ء
  s! \`${prefix}clan room\`: لعمل روم شات او كتابي بأسم الكلان ( صاحب الكلان فقط ) ء`)
...
client.on('ready', function(){
    var ms = 10000 ;
    var setGame = [' s!help',`Im on ${client.guilds.size} servers`,`i help ${client.users.size} Members`,' BAD Is The Best ','Just Do It'];
    var i = -1;
    var j = 0;
...
client.on("message", message => {
    ...
    .setDescription(`
    أمثله على الأوامر :
    s!role @mention rolename : لأعطاء رتبة لعضو معين
    s!role all rolename : لأعطاء رتبة للجميع
    s!role humans rolename : لأعطاء رتبة للاشخاص فقط
    s!role bots rolename : لأعطاء رتبة لجميع البوتات`)
...
client.on("message", (message) => {
   ...
   if (message.content.startsWith("s!new")) {
...
client.on('message', function(msg) {
 ...
 client.on('message', message => {
   if (message.content.startsWith("s!cuttweet")) {
                if(!message.channel.guild) return message.reply('** This command only for servers**');
  var embed = new Discord.RichEmbed()
  .setColor('RANDOM')
   .setThumbnail(message.author.avatarURL)
...
const Sra7a = [
   ...
   client.on('message', message => {
       if (message.author.bot) return;
 if (message.content.startsWith('s!sara7a')) {
     if(!message.channel.guild) return message.reply('** This command only for servers **');
...
message.react("❌")
  ...
  if(message.content.startsWith(prefix + "gRole")) {
    ...
    .addField('s! اسم الرتبة',role.name,true)
    .addField('s! اي دي الرتبة',role.id,true)
    .addField('s! تم انشاء الرتبة',role.createdAt.toLocaleString(),true)
    .addField('s! لون الرتبة',role.hexColor,true)
    .addField('s! عدد الاعضاء الذي لديهم نفس الرتبة',role.members.size,true)
    .addField('s! مركز الرتبة بين كل الرتب',role.position - message.guild.roles.size,true)
    .addField('s! خصائص الرتبة',role.permissions,true)
...
**
『s!ping ====> لمعرفة سرعة اتصال البوت
『s!link ====> يسويلك رابط لمدة يوم وعدد الاستخدامات 100
『s!tag ====> لعرض الكلام بشكل جميل و كبير
『s!تقديم
『s!quran ====> لعرض 200 صفحة من القرآن الكريم
『s!perms ====> لعرض صلاحياتك
『s!topinv ====> لعرض صاحب اكثر دعوات 
『s!hypixel ====> لرؤية احصائياتك في هايبكسل
『s!contact ====> للتواصل مع صاحب البوت
『s!القبيلة ====> لعبة الكلانات
『s!report ====> للابلاغ عن احد
『s!invite ====> لمعرفة عدد دعواتك
『s!color ====> لوضع لون لك
『s!emojilist ====> لرؤية قائمة اموجيات السيرفر
『s!date ====> يعرضلك تاريخ اليوم
『s!email ====> يعطيك ايميل و باس عشوائي
『s!gRole Role name ====> يوريك معلومات رتبة باسمها او الايدي حقها
『s!bot ====> يعرض لك كل معلومات البوت
『s!say ====> يكرر الكلام الي تكتبو
『s!savatar ====> صورة السيرفر
『s!id ====> معلومات عنك
『s!new ====> يفتح لك تكت
『s!avatar ====> صورتك او صورة الي تمنشنو
『s!embed ====> يكرر الي تقولو بشكل حلو
『s!rules ====> يعرض لك قوانين السيرفر
『s!v2min ====> لصنع روم صوتي مؤقت
『s!inv ====> لدعوة البوت الى سيرفرك
『 By ====> SniperBot
**
`)
...
**
『s!bc ====> رسالة لجميع اعضاء السيرفر
『s!bk ====> رسالة لجميع اعضاء السيرفر 2
『s!rolebc ====> رسالة لرتبة معينة
『s!setbot ====> لعمل روم صوتي بعدد البوتات في السيرفر
『s!bans ====> الاعضاء المبندين من سيرفرك
『s!setmember ====> لعمل روم صوتي بعدد اعضاء السيرفر
『s!cto ====> تغير الكلام في اعلى الروم
『s!leave ====> لطرد البوت من سيرفرك
『s!setMedia ====> لتحديد روم الصور
『s!ranks ====> يوريك رتب السيرفر
『s!vonline ====> لعمل روم صوتي اونلاين
『s!schannel ====> اضهار الشات المخفية
『s!sr Exemple ====> لتغيير اسم السيرفر
『s!kv ====> لطرد عضو من روم صوتي
『s!hchannel ====> اخفاء الشات
『s!talk ====> للتكلم بصفة البوت
『s!count ====> عدد اعضاء السيرفر
『s!setlog ====> لصنع روم اللوج
『s!server ====> يعرض لك معلومات عن السيرفر
『s!movall ====> لسحب الجميع الى رومك
『s!sug ====> suggestions لصنع اقتراح لازم روم 
『s!bs ====> لمعرفة سيرفرات البوت
『s!rooms ====> لرؤية رومات السيرفر
『s!dc ====> مسح كل الرومات
『s!dr ====>  فوق كل الرانكات
『s!allbots ====> يوريك كل البوتات في سيرفرك
『s!move ====> لسحب عضو الى روم صوتية
『s!giveaway ====> لصنع جيفواي
『s!role @mention rolename ====> لأعطاء رتبة لعضو معين
『s!role all rolename ====> لأعطاء رتبة للجميع
『s!role humans rolename ====> لأعطاء رتبة للاشخاص فقط
『s!role bots rolename ====> لأعطاء رتبة لجميع البوتات
『s!members ====> حالات اعضاء السيرفر
『s!send ====> ارسال رسالة لشخص المنشن
『s!clr <numbr> ====> مسح الشات بعدد
『s!clear ====> مسح الشات
『s!mute @user <reason> ====> اعطاء العضو ميوت لازم رتبة <Muted>
『s!unmute @user ====> لفك الميوت عن الشخص 
『s!kick @user <reason> ====> طرد الشخص من السيرفر
『s!ban @user <reason> ====> حضر الشخص من السيرفر
『s!unban @user ====> لفك حضر الشخص من السيرفر
『s!mutechannel ====> تقفيل الشات
『s!unmutechannel ====> فتح الشات
『s!ct <name> ====> انشاء شات
『s!cv <name> ====> انشاء رووم فويس
『 By ====>SniperBot
**
`)
   message.author.sendEmbed(embed)
 ...
 **       
『s!8ball ====> لعبه تسال البوت اسال  وهو يجاوب عنها
『s!cuttweet ====> لعبة كت تويت
『s!هل تعلم
『s!cal ====> ألة حاسبة
『s!za5 ====> لزخرفة ما تقول
『s!cats ====> قطط كيوت
『s!love ====> يعطيك اقوال عن الحب 
『s!حكم
『s!جمع
『s!طرح
『s!ضرب
...
client.on('message', message => {
if(message.content.startsWith("s!فواكه")) {
  let slot1 = ['🍏', '🍇', '🍒', '🍍', '🍅', '🍆', '🍑', '🍓'];
...
client.on('message', msg => {
    if(msg.content.startsWith('s!binv')) {
...
var prefix = "s!";
    ...
    if (message.content === prefix + "date") {
        ...
        var currentTime = new Date(),
            ...
            .setColor('RANDOM')
            .setTimestamp()
            .setDescription( ""+ Day + "s!" + Month + "s!" + Year + "")
...
if (msg.content.split(" ")[0].toLowerCase() === "s!cr") {
...
client.on('message', message => {
          let args = message.content.split(' ').slice(1);
   if(message.content.split(' ')[0] == 's!color'){
...
client.on('message', function(message) {
    ...
    .setTitle("`New Report!`")
    .setThumbnail(message.author.avatarURL)
    .addField("**# s! أبلغ المستخدم:**",mUser,true)
    .addField("**# s! معرف المستخدم المبلغ عنه:**",mUser.id,true)
    .addField("**# s! السبب:**",messageReason,true)
    .addField("**# s! Channel:**",message.channel,true)
    .addField("**# s! Time:**",message.createdAt,true)
    .setFooter("تحذير !!! سيتم معاقبة صاحب البلاغ اذ كان البلاغ كاذب")
message.channel.send(Rembed)
message.channel.send("__هل انت متكد انك تريد ارسل البلاغ الا صاحب المسؤول??__").then(msg => {
    msg.react("✅")
    msg.react("❌")
.then(() => msg.react('❌'))
.then(() =>msg.react('✅'))
...
reaction1.on("collect", r => {
    ...
    message.reply("**# s! Done! 🎇**");
})
reaction2.on("collect", r => {
    message.reply("**# s! Canceled!**");
})
})
}
});
